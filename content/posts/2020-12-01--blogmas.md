...
This Blogmas post is getting obscenely long so I implemented a back to top button that hangs around the buttom right corner of a post.
...
The mobile button is different from the desktop version. I toyed with just having the icon have the shadow, but I felt like it didn't stand out enough on mobile because it was on top of white text. The desktop version is just the chevron to the right of the text, completely flat. The icon has a shadow on hover (not the entire button). I wanted this to be consistent with the home button in the top left corner.

### Implementation

All the CSS isn't very interesting -- except for that of course it's `position: fixed` and that I had to use `calc` for the desktop version to ensure that it was the right distance from the middle content.
