...
- [🎨 **12/22**: Quarantine running recap](#-1222-quarantine-running-recap)
- [📚 **12/23**: Redux](#-1223-redux)
...
Pretty simple, right? You can peep the [GitHub repo](https://github.com/karenying/progressive-image-loader).
...
Every component depends on the "stack" of colors which was stored as state variables in `App.js`. I had no concept of state management and drilled the F outta those poor components. You can peep the [GitHub repo](https://github.com/karenying/gradient.png) if you wanna vom 🤮

7 months later, thankfully I've added [React Context](https://reactjs.org/docs/context.html) and [MobX](https://mobx.js.org/README.html) to my state management tool belt. So today, I took a stab at learning another popular option, [React Redux](https://react-redux.js.org/).
...
**The twist**: To have a text input box on the main page that if the user typed a valid hex color code into it and hit the enter key, the text in the app would change to this color.
...
- There's one reducer -- it takes in a string, and sets the state's hex value to that
- Export this reducer, `changeHex`, so that we can use it with my input component
- Export `selectHex` which a selector that keeps track of the current hex
...
const HexInput = () => {
  ...
  return <input onKeyDown={(e) => handleKeyDown(e)}></input>;
};
...
- Then we convert that to a valid hex string that inline-styling accepts and style our `App` component

It worked as expected and I've successfully crash coursed through Redux.

### Overall thoughts

Context >> Redux >> MobX

Context is probably better for super simple global states -- toggling light / dark mode is a classic example. This text color state was simple enough that Redux seemed a overkill but I could definitely see it being useful for more complex state management.

I've only touched the tip of the Redux iceberg but seems pretty okay to me. It's so well documented that I'm sure it would be nice to use.
