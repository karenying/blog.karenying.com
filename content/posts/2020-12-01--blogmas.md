...
**Side note**: `scroll-behavior: smooth` has [better browser compatibility](https://developer.mozilla.org/en-US/docs/Web/CSS/scroll-behavior) than `window.scrollTo` but would require me to code the scroll to top JS from scratch. So that's just gonna be backlogged for now.
...
- We clear the previous timer and set a new one for 50ms
- After 50ms, if a new scroll hasn't been detected, then the div's opacity is restored to `1`
