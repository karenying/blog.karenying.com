...
- `isScrolling` makes sure that `updateScrollDir` is only called once per repaint -- the code works without `requestAnimationFrame` business, it's basically just acting as throttling so I guess I'll keep it
