...
While the using a state variable triggers a rerender every time and using refs does not, the first implementation is considered the "correct" React way. It's [declarative](https://fjorgedigital.com/insights/blog/the-philosophy-of-react-declarative-rendering/) -- in the first case, it declares what the opacity should be and the state variable triggers the rerender, instead of directly setting the opacity with `ref.current`.
