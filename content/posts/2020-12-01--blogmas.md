...
A trickier thing was getting the button to hide when the user is scrolling. This is for mobile only. I tried it on desktop but it seemed unnecessary since the button wasn't over text. This is what I ended up doing:
...
Hiding the back to top button on every scroll was a lil distracting. Instead, I want it to be hidden if the user is scrolling _down_, but visible if the user scrolls _up_. I'm also making the home button appear at the top simultaneously.
...
- In a separate `useEffect`, I set the opacity of the home and back to top buttons based on `isScrollingUp`
