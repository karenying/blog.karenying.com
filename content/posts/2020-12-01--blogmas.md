...
This Blogmas post is getting obscenely long so I implemented a back to top button that hangs around the bottom right corner of posts.
...
A trickier thing was getting the button to hide when the user is scrolling. This is for mobile only. I tried it on desktop but it was a bit too distracting and seemed unnecessary since the button wasn't over text. This is what I ended up doing:
...
While the using a state variable triggers a rerender every time and using refs does not, the first implementation is considered the "correct" React way. It's [declarative](https://fjorgedigital.com/insights/blog/the-philosophy-of-react-declarative-rendering/) -- in the first case, it declares what the opacity should be and the state variable helps change the CSS, instead of directly setting the opacity with `ref.current`.

Either way, I'm happy with how it turned out!

**12/12 Update**: I was not happy with how it turned out and hence this next post 😡

## ⚙️ **12/12**: Back to top button (part 2)

_Implemented an hour before my second final exam_

Hiding the back to top button on every scroll was a bit too distracting. Instead, I want it to be hidden if the user is scrolling _down_, but visible if the user scrolls _up_. I'm also making the home button appear at the top simultaneously.
...
I added a border to both because they didn't seem very visible without.
...
I replaced the previous scroll handler with this (I stole 98% of this code):
...
And I added another `useEffect` to set opacity:
...
Let's break this down (I'm leaning on the Stack Overflow explanation too):
...
- `isScrolling` makes sure that `updateScrollDir` is only called once per repaint
- In a separate `useEffect`, I set the opacity based on `isScrollingUp`

This is hopefully the last part of the back to top button saga!
