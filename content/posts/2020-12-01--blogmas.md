...
\***12/13 Update**: UmmMMMmMmMm this plugin has a race condition I swear so I'm reverting back to my old ways of just relying on Medium. On production, I would click a post or click next page and nothing would render. The console would say that the `readingTime` was undefined in the GraphQL query which would cause the page to crash. I'll have to investigate more but I'm pretty disappointed. Maybe I did something wrong idk
