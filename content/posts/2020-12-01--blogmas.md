...
- [⚙️ **12/13**: Use plugin to calculate reading time](#️-1213-use-plugin-to-calculate-reading-time)
...
In practice, perhaps only the right side (posts) would be abstracted but I just wanted to simplify as much as possible! And since my site is static, this wouldn't really be applicable anyways haha.
...
## ⚙️ **12/13**: Use plugin to calculate reading time

_Implemented between my **third** and **fourth** finals_

Yay we're officially more than halfway through Blogmas! Ngl, this + finals + FT work this week has been quite a LOT.

Today's just gonna be quick and easy. Before, I calculated time read by literally copy and pasting a post into a Medium post draft and stealing their calculation (or just taking it if the post was already on Medium). I had a `minutes` field in the frontmatter of every post and would query it. Instead of this jank approach, I using the `gatsby-remark-reading-time` [plugin](https://www.gatsbyjs.com/plugins/gatsby-remark-reading-time-v2/).

I just followed the installation instructions and it worked without any hiccups.
...
Here are the posts where there's a difference between Medium and this plugin:
...
From this we can see that images increase Medium's calculated time, whereas codeblocks increase the plugin's (on Medium I embedded GitHub gists).
