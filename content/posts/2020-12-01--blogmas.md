...
- [🎨 **12/12**: Back to top button](#-1212-back-to-top-button)
...
\***12/8 Update**: merged a PR for [work](https://www.wellsheet.com/) today that animated a skeleton screen for a loading state! I lifted code from this [CodePen](https://codepen.io/viktorstrate/pen/yoBRLy).
...
_Blogged while listening to my [Top Songs 2020 playlist](https://open.spotify.com/playlist/37i9dQZF1EMaba5daAs6pY?si=wFKQvUuVT7m7rKz6UlG0BQ)_
...
2020 was definitely a big coding year for me! I switched my major to CS at the end of 2019, so this year has just been me diving into the world of web dev. I worked on some full stack stuff and realized I definitely have a preference for frontend and an interest in product / design.
...
## 🎨 **12/12**: Back to top button

This Blogmas post is getting obscenely long so I'm implementing a back to top button that hangs around the buttom right corner of a post.

I decided on an upwards chevron from [Ionicons](https://ionicons.com/) but I'm using it through [React Icons](https://react-icons.github.io/react-icons/search?q=arrow). **Side note**: At some point I wanna stop using React Icons and just import the SVGS I wanna use but I'm lazy.

This is the design I settled on for mobile:

![](/media/blogmas/backtotop.png#width=300)

The mobile button is different from the desktop version. I toyed with just having the icon have the shadow, but I felt like it didn't stand out enough on mobile because it was on top of the text. The desktop version is just the chevron to the right of the content text, completely flat, but the chevron has a shadow on hover (not the entire button). I wanted this to be consistent with the home button in the top left corner.

### Implementation

All the CSS probably isn't interesting -- except for that of course it's `position: fixed` and that I had to use `calc` for the desktop version to ensure that it was the right distance from the middle content.

Add the back to top on click wasn't hard either:
...
A trickier thing was getting the button to hide when the user is scrolling. This is what I ended up doing:
...
- First, we need a ref to the back to top div because we're manipulating its style
- On mount, we add an event listener for scrolling -- `onScroll` is called everytime the user scrolls
...
- We clear the previous timer and set a new one that triggers after 100ms
- After 100ms, if a new scroll hasn't been detected, then the div's opacity is set back to 1
- On unmount, we remove the scroll listener and clean up the timeout

Another way I could have done this is using state variables instead of refs I think:

```jsx
const [opacity, setOpacity] = 1;
let timeoutID;
