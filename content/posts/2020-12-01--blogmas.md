...
For our grid container, we'll declare each square to be `50px` big and the gap between them to be `12px`:
...
Each div has a color class name as well as `a` if it's a square, or `b` if it takes up two squares.
...
`.blue.a` only takes up one square, so we only need to declare its row and column which are `1` and `2` respectively (Grid is 1-indexed):

![slack](/media/blogmas/slack-grid.png#width=300px)

`.blue.b` is in the 2nd row so we put `2` for that. Then for `column`, we use `start / finish` -- the div extends _up to_ `finish`. `.blue.b` starts at column 1 and ends right before column 3. Thus, we have `1 / 3`.

We can fill out the rest of the colors by examining which boxes they lie in. I also added more CSS to configure the colors and rounded borders. You can see the demo on [CodePen](https://codepen.io/karenying/pen/gOwPZYZ) and the full code on [GitHub](https://github.com/karenying/css-grid-slack-logo).
...
Grid is pretty easy to use! It definitely has a ton of other features that I didn't explore in this simple render. It makes sense to use it for 2D layouts instead of just nesting Flexbox (@me oops). I bet I will visit CSS-Tricks' [Grid guide](https://css-tricks.com/snippets/css/complete-guide-grid/) frequently in the future as I start to use Grid more often.
